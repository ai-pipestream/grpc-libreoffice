...
- `StreamPages(stream StreamPagesRequest) returns (stream StreamPagesResponse)`.
  ...
  pixel dimensions and effective DPI), then one final `RenderStatus`.
...
structure schema (tracking docling-core v2 for interoperability). Future
typed extraction events (tables, embedded images) will emit into that shape.
...
(`libreofficekit-dev`), and LibreOffice itself at runtime. gRPC is fetched
and built by CMake.
